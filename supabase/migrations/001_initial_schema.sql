-- ═══════════════════════════════════════════════════════════════
-- Monitor Polític Municipal — Castell-Platja d'Aro
-- Migració completa de la base de dades
-- Executa aquest fitxer al Supabase SQL Editor
-- ═══════════════════════════════════════════════════════════════
...
CREATE POLICY "Admin veu tots els usuaris" ON usuaris
  FOR ALL USING (
    EXISTS (SELECT 1 FROM usuaris WHERE id = auth.uid() AND role = 'admin')
  );

-- ── Taula principal de monitoratge ────────────────────────────
CREATE TABLE IF NOT EXISTS monitoratge (
  ...
  fts                      TSVECTOR GENERATED ALWAYS AS (
    to_tsvector('simple', unaccent(COALESCE(titol,'') || ' ' || COALESCE(resum,'') || ' ' || COALESCE(font,'')))
  ) STORED
);
...
CREATE INDEX IF NOT EXISTS idx_mon_fts        ON monitoratge USING GIN(fts);
CREATE INDEX IF NOT EXISTS idx_mon_estat      ON monitoratge(estat_seguiment);

ALTER TABLE monitoratge ENABLE ROW LEVEL SECURITY;
CREATE POLICY "Usuaris autenticats llegeixen" ON monitoratge
  FOR SELECT USING (auth.role() = 'authenticated');
CREATE POLICY "Usuaris editen observacions" ON monitoratge
  FOR UPDATE USING (auth.role() = 'authenticated')
  WITH CHECK (TRUE); -- La lògica de camps es controla al backend
CREATE POLICY "Admin i service editen tot" ON monitoratge
  FOR ALL USING (
...
CREATE TABLE IF NOT EXISTS expedients_bpm (
  ...
  data_deteccio       TIMESTAMPTZ DEFAULT NOW(),
  fts                 TSVECTOR GENERATED ALWAYS AS (
    to_tsvector('simple', unaccent(COALESCE(titol,'') || ' ' || COALESCE(contingut,'') || ' ' || COALESCE(assumpte,'')))
  ) STORED
);
...
CREATE INDEX IF NOT EXISTS idx_bpm_any     ON expedients_bpm(any);
...
CREATE INDEX IF NOT EXISTS idx_bpm_fts     ON expedients_bpm USING GIN(fts);
...
CREATE POLICY "Llegir compromisos" ON compromisos FOR SELECT USING (auth.role() = 'authenticated');
CREATE POLICY "Admin gestiona compromisos" ON compromisos FOR ALL USING (
  EXISTS (SELECT 1 FROM usuaris WHERE id = auth.uid() AND role = 'admin')
);

ALTER TABLE comentaris_compromisos ENABLE ROW LEVEL SECURITY;
CREATE POLICY "Llegir comentaris" ON comentaris_compromisos FOR SELECT USING (auth.role() = 'authenticated');
CREATE POLICY "Usuaris afegeixen comentaris" ON comentaris_compromisos FOR INSERT WITH CHECK (auth.uid() = created_by);
CREATE POLICY "Admin gestiona comentaris" ON comentaris_compromisos FOR ALL USING (
  EXISTS (SELECT 1 FROM usuaris WHERE id = auth.uid() AND role = 'admin')
);

-- ── Funció stats del dashboard ────────────────────────────────
...
BEGIN
  SELECT json_build_object(
    ...
    'urgents_setmana', (
      SELECT COUNT(*) FROM monitoratge
      WHERE classificacio = 'URGENT'
      AND data_deteccio >= NOW() - INTERVAL '7 days'
    ),
    'venciments_7dies', (
      SELECT COUNT(*) FROM monitoratge
      WHERE venciment IS NOT NULL
      AND venciment BETWEEN CURRENT_DATE AND CURRENT_DATE + INTERVAL '7 days'
    ),
    'pendents_90dies', (
      SELECT COUNT(*) FROM monitoratge
      WHERE estat_seguiment = 'pendent'
      AND data_deteccio < NOW() - INTERVAL '90 days'
    ),
    'compromisos_incomplerts', (
      SELECT COUNT(*) FROM compromisos
      WHERE estat IN ('pendent', 'incomplert')
    )
  ) INTO result;
  RETURN result;
END;
...
-- ── Trigger: set admin per a aitor.tendero@gmail.com ──────────
...
-- ── Inserir admin si no existeix ──────────────────────────────
-- (Executa manualment si l'admin ja s'ha registrat sense el trigger)
-- UPDATE usuaris SET role = 'admin', aprovat = TRUE WHERE email = 'aitor.tendero@gmail.com';
