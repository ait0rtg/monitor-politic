...
export async function POST(req: Request) {
  ...
  const words = pregunta.split(' ').filter(Boolean).join(' | ')

  const { data: documents } = await supabase
    .from('monitoratge')
    .select('titol, resum, font, data_deteccio, url_original, import_detectat, venciment, tema_principal')
    .or(`titol.ilike.%${pregunta}%,resum.ilike.%${pregunta}%`)
    .limit(20)

  const { data: expedients } = await supabase
    .from('expedients_bpm')
    .select('numero, titol, contingut, assumpte, data, url_document, tema')
    .or(`titol.ilike.%${pregunta}%,contingut.ilike.%${pregunta}%,assumpte.ilike.%${pregunta}%`)
    .limit(15)

  const contextDocs = (documents || []).map(d =>
    '[' + d.font + ' - ' + (d.data_deteccio?.split('T')[0] || '') + ']\nTitol: ' + d.titol + '\nResum: ' + (d.resum || 'Sense resum') + '\nImport: ' + (d.import_detectat ? d.import_detectat + ' euros' : 'No detectat') + '\nURL: ' + d.url_original
  ).join('\n\n---\n\n')
  ...
  if (!context.trim()) {
    return NextResponse.json({
      resum_executiu: 'No s\'han trobat documents relacionats amb aquesta consulta.',
  ...
  const completion = await openai.chat.completions.create({
    ...
    temperature: 0.2,
    messages: [
      { role: 'system', content: SYSTEM_PROMPT_ASSISTENT },
...
function parseAssistentResponse(content: string) {
  const extract = (from: string, to: string) => {
    const pattern = new RegExp(from + ':\\s*([\\s\\S]+?)(?=' + to + ':|$)')
    const m = pattern.exec(content)
    return m ? m[1].trim() : ''
  }

  const preguntesM = /PREGUNTES SUGGERIDES:\s*([\s\S]+?)(?=DOCUMENTS FONT:|$)/.exec(content)
  const preguntesText = preguntesM ? preguntesM[1] : ''
